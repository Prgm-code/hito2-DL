...
export function createDestinationArtwork(location: Location, residents: Character[]): HTMLDivElement {
  ...
  if (residents.length) {
    ...
    artwork.append(createElement("span", { className: "photo-fallback", text: "◎", attrs: { "aria-hidden": "true" } }));
...
export function createDestinationCard(
  ...
  const dimension = createElement("p", { className: "dimension" },
    createSvg("0 0 24 24", [
  ...
  const content = createElement("div", { className: "card-content" },
    ...
    createElement("div", { className: "card-footer" },
      createElement("span", {}, createElement("b", { text: location.residents.length }), " residentes"),
      chooseButton,
    ),
  );
  return createElement("article", { className: "destination-card" }, createDestinationArtwork(location, residents), content);
}
...
export function createReservationItem(
  ...
  const meta = createElement("div", { className: "reservation-meta" },
    ...
    createElement("span", { text: `${reservation.passengers} pasajero${reservation.passengers === 1 ? "" : "s"}` }),
  );
  ...
  const side = createElement("div", { className: "reservation-side" },
    createElement("span", {
...
export function appendDestinationHint(target: HTMLElement, location: Location): void {
  const route = createElement("span", {}, createElement("b", { text: location.type }), ` · ${location.dimension}`);
  target.replaceChildren(route, createElement("span", { text: `${location.residents.length} residentes` }));
}
